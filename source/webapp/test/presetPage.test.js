describe("check preset recipe1 can fill the input correctly", () => {
   beforeAll(async () => {
      await page.goto('https://alien-traveler.github.io/cse110-fa22-group39/source/webapp/presetList/presetList.html');
   ...
   it('make sure coffee name is "Coffe Americano', async () => {
      const coffeeName = await page.evaluate(x => x.value, await page.waitForXPath('//*[@id="name"]'));
   ...
   it('make sure coffee type is "hot"', async () => {
      ...
      const hotCheckboxVal = await (await hotCheckbox.getProperty('checked')).jsonValue();
      expect(hotCheckboxVal).toBe(true);

      const coldCheckbox = await page.waitForXPath('//*[@id="cold-coffee"]');
      const coldCheckboxVal = await (await coldCheckbox.getProperty('checked')).jsonValue();
      expect(coldCheckboxVal).toBe(false);
   })

   it('make sure drink is "Espresso"', async () => {
      const drink = await page.evaluate(x => x.value, await page.waitForXPath('//*[@id="drinks-name"]'));
      expect(drink).toBe("Espresso");
   })

   it('make sure size is "M', async () => {
      const size = await page.evaluate(x => x.value, await page.waitForXPath('//*[@id="size-name"]'));
      expect(size).toBe("M");
   })

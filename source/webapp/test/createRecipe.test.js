describe('Test customize recipe', () => {
   ...
   beforeAll(async () => {
      ...
      await page.goto('https://alien-traveler.github.io/cse110-fa22-group39/source/webapp/');
      await page.goto(
      ...
      // go to review page
      await (await page.$x('//*[@id="customize-form"]/div/div[10]/input'))[0].click();
   });
   ...
   it('test the customized process', async () => {
      ...
      expect(await page.evaluate(() => document.getElementById('coffee-name').value)).toBe(COFFEE_NAME);
      expect(await page.evaluate(() => document.getElementById('custom-coffee-name').value)).toBe(TYPE);
      expect(await page.evaluate(() => document.getElementById('drink-type-input').value)).toBe(DRINK);
      expect(await page.evaluate(() => document.getElementById('size-type-input').value)).toBe(SIZE);

      const caramelCheckbox = await page.waitForXPath('//*[@id="caramel"]');
      const carmelCheckboxVal = await (await caramelCheckbox.getProperty('checked')).jsonValue();
      expect(carmelCheckboxVal).toBe(false);

      const sugarCheckbox = await page.waitForXPath('//*[@id="sugar"]');
      const sugarCheckboxVal = await (await sugarCheckbox.getProperty('checked')).jsonValue();
      expect(sugarCheckboxVal).toBe(false);

      const extraShotCheckbox = await page.waitForXPath('//*[@id="extra-shot"]');
      const extraShotCheckboxVal = await (await extraShotCheckbox.getProperty('checked')).jsonValue();
      expect(extraShotCheckboxVal).toBe(false);

      const milkCheckbox = await page.waitForXPath('//*[@id="milk"]');
      const milkCheckboxVal = await (await milkCheckbox.getProperty('checked')).jsonValue();
      expect(milkCheckboxVal).toBe(true);

      const creamCheckbox = await page.waitForXPath('//*[@id="cream"]');
      const creamCheckboxVal = await (await creamCheckbox.getProperty('checked')).jsonValue();
      expect(creamCheckboxVal).toBe(true);

      const waterCheckbox = await page.waitForXPath('//*[@id="water"]');
      const waterCheckboxVal = await (await waterCheckbox.getProperty('checked')).jsonValue();
      ...
      let name = await page.$eval('body > table > tbody > tr:nth-child(2) > td:nth-child(1) > div', x => x.innerText);
      expect(name).toBe(COFFEE_NAME);

      // delete
      await (await page.$x(inputXPath))[0].type('');
      await page.click('#remove0');
      let tableHtml = await page.evaluate(() => document.querySelector('body > table > tbody').innerHTML);
      expect(tableHtml).not.toContain(COFFEE_NAME);

      // search the deleted coffee again
      await (await page.$x(inputXPath))[0].type(COFFEE_NAME);
      tableHtml = await page.evaluate(() => document.querySelector('body > table > tbody').innerHTML);
      expect(tableHtml).not.toContain(COFFEE_NAME);
   })
})

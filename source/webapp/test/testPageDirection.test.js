describe("Testing for repeated recipe names", () => {
    ...
    it('The page should display the same contents as before by going back',     async function () {
        ...
        expect (await page.evaluate(() => document.getElementById("name").value)).toBe(COFFEE_NAME);
        expect (await page.evaluate(() => document.getElementById("hot-coffee").checked)).toBe(true);
        expect (await page.evaluate(() => document.getElementById("drinks-name")[2].value)).toBe(DRINK);
        expect (await page.evaluate(() => document.getElementById("size-name")[3].value)).toBe(SIZE);
        ...
        const milkCheckboxVal = await (await milkCheckbox.getProperty("checked")
        ).jsonValue();
